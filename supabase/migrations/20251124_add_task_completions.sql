...
ALTER TABLE public.task_completions
    ADD CONSTRAINT task_completion_target_check CHECK (
        (medication_id IS NOT NULL) OR (calendar_event_id IS NOT NULL)
    );
...
CREATE INDEX IF NOT EXISTS idx_task_completions_event ON public.task_completions (calendar_event_id);
